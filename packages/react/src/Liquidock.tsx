...
export interface LiquidockDockProps {
  className?: string;
  style?: CSSProperties;
  onAction?: (tool: BaseToolContract, runtime: DockRuntime) => void;
}

export interface LiquidockProps extends LiquidockProviderProps, LiquidockDockProps {}

function useDockRuntimeState(): RuntimeState {
  const runtime = useContext(LiquidockContext);

  if (!runtime) {
    throw new Error('Liquidock components must be used within a LiquidockProvider');
  }
...
export function LiquidockProvider({
  children,
  runtime: providedRuntime,
  config,
  tools,
}: LiquidockProviderProps) {
  ...
  return <LiquidockContext.Provider value={runtime}>{children}</LiquidockContext.Provider>;
}

export function LiquidockDock({ className, style, onAction }: LiquidockDockProps) {
...
export function Liquidock({ children, runtime, config, tools, className, style, onAction }: LiquidockProps) {
  return (
    <LiquidockProvider config={config} runtime={runtime} tools={tools}>
      {children}
      <LiquidockDock className={className} onAction={onAction} style={style} />
    </LiquidockProvider>
  );
}
